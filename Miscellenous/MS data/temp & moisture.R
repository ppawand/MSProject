...
temp <- subset(temp, temp$temperature > (Q1 - 1.5*IQR) & 
                 temp$temperature < (Q3 + 1.5*IQR))
...
moist <- subset(moist, moist$moisture > (Q1m - 1.5*IQRm) & 
                  moist$moisture < (Q3m + 1.5*IQRm))
...
# summarizing daily temperatures
...
daily.TM$month <- factor(daily.TM$month, 
...
# precipitation
#####################
precipitation <- read_excel("~/Desktop/MS Research/MS data/weather.data.xlsx")
str(precipitation)
...
# summarizing temperature and moisture by month
...
# refitting best model
moist_model <- lmer(moisture ~ warming * residue * irrigation + (1|block) + 
                      (1|month), REML = F, data = monthly.avg.TM)
...
moiturevstemp <- lmer(moisture ~ temp + (1|block) + (1|month), 
                      data = monthly.avg.TM, REML = F)
...
air.temp.average$warming <-ifelse(air.temp.average$trt %in% c("C", "R"),
                                  "Control", "OTC")
air.temp.average$residue <-ifelse(air.temp.average$trt %in% c("C", "W"), 
                                  "noResidue", "Residue")

air.temp.average[, c(1:3, 6,9,10)] <- 
  lapply(air.temp.average[, c(1:3, 6, 9, 10)], factor)
air.temp.average$trt <- factor(air.temp.average$trt, 
                               levels = c("C", "W", "R", "WR"))
air.temp.average$month <- factor(air.temp.average$month, 
...
weekly.air.temperature$week <- strptime(weekly.air.temperature$week, 
                                        format  = "%Y-%m-%d")
...
temp_light_model <- lmer(temp ~ light + (1|block) + (1|month),
                        REML = F,
                        data = airtemp.scaled)

Anova(temp_light_model)
summary(temp_light_model)
plot(temp_light_model)
qqnorm(residuals(temp_light_model))
hist(residuals(temp_light_model))
r.squaredGLMM(temp_light_model)
