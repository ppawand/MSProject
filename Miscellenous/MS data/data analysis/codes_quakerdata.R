...
ggplot(nutrients, aes(temp, OM)) +
  geom_point(aes(color = irrigation, fill  = irrigation, shape = trt)) +
...
# mbc and nitrate
ggplot(mbc, aes(nitN, mbc, color = irrigation)) + 
...
# mbc vs P
ggplot(mbc, aes(P, mbc, color = irrigation)) + 
  geom_point(aes(shape = trt, fill = irrigation)) +
  geom_smooth(method = "lm", se = F) +
  scale_color_manual(labels = c("Dryland", "Irrigated"),
                     values = c("red3", "blue3")) +
